...
func (p *OAuth2Provider) findClientPublicJWK(ctx context.Context, oidcClient OpenIDConnectClient, t *jwt.Token, expectsRSAKey bool) (interface{}, error) {
...
func (p *OAuth2Provider) AuthenticateClient(ctx context.Context, r *http.Request, form url.Values) (Client, error) {
	...
	return p.DefaultClientAuthenticationStrategy(ctx, r, form)
}

// DefaultClientAuthenticationStrategy provides the oauth21's default client authentication strategy,
// HTTP Basic Authentication and JWT Bearer
func (p *OAuth2Provider) DefaultClientAuthenticationStrategy(ctx context.Context, r *http.Request, form url.Values) (Client, error) {
	if assertionType := form.Get("client_assertion_type"); assertionType == clientAssertionJWTBearerType {
		...
		} else if len(p.Config.GetTokenURLs(ctx)) == 0 {
		...
		} else if p.Storage.ClientAssertionJWTValid(ctx, jti) != nil {
		...
		if err := p.Storage.SetClientAssertionJWT(ctx, jti, time.Unix(expiry, 0)); err != nil {
			return nil, err
		}

		if !audienceMatchesTokenURLs(claims, p.Config.GetTokenURLs(ctx)) {
			return nil, errorsx.WithStack(ErrInvalidClient.WithHintf(
				...
				strings.Join(p.Config.GetTokenURLs(ctx), "' or '")))
		}
...
func (p *OAuth2Provider) checkClientSecret(ctx context.Context, client Client, clientSecret []byte) error {
	var err error
	err = p.Config.GetSecretsHasher(ctx).Compare(ctx, client.GetHashedSecret(), clientSecret)
	if err == nil {
		return nil
	}
	...
	for _, hash := range cc.GetRotatedHashes() {
		err = p.Config.GetSecretsHasher(ctx).Compare(ctx, hash, clientSecret)
		if err == nil {
			return nil
		}
	}
