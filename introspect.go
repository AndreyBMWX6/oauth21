...
func (p *OAuth2Provider) IntrospectToken(ctx context.Context, token string, tokenUse TokenUse, session Session, scopes ...string) (_ TokenUse, _ AccessRequester, err error) {
	ctx, span := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/AndreyBMWX6/oauth21").Start(ctx, "OAuth2Provider.IntrospectToken")
	...
	for _, validator := range p.Config.GetTokenIntrospectionHandlers(ctx) {
