...
func (p *OAuth2Provider) NewRevocationRequest(ctx context.Context, r *http.Request) (err error) {
	ctx, span := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/AndreyBMWX6/oauth21").Start(ctx, "OAuth2Provider.NewRevocationRequest")
	...
	client, err := p.AuthenticateClient(ctx, r, r.PostForm)
	if err != nil {
		return err
	}
	...
	var found = false
	for _, loader := range p.Config.GetRevocationHandlers(ctx) {
...
// is already achieved.
func (p *OAuth2Provider) WriteRevocationResponse(ctx context.Context, rw http.ResponseWriter, err error) {
