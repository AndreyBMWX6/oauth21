...
func (p *OAuth2Provider) WriteAuthorizeResponse(ctx context.Context, rw http.ResponseWriter, ar AuthorizeRequester, resp AuthorizeResponder) {
	...
	case ResponseModeFormPost:
		...
		WriteAuthorizeFormPostResponse(redir.String(), resp.GetParameters(), GetPostFormHTMLTemplate(ctx, p), rw)
		return
	...
	default:
		if p.ResponseModeHandler(ctx).ResponseModes().Has(rm) {
			p.ResponseModeHandler(ctx).WriteAuthorizeResponse(ctx, rw, ar, resp)
			return
		}
	}
}
