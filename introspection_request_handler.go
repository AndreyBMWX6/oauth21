...
func (p *OAuth2Provider) NewIntrospectionRequest(ctx context.Context, r *http.Request, session Session) (_ IntrospectionResponder, err error) {
	ctx, span := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/AndreyBMWX6/oauth21").Start(ctx, "OAuth2Provider.NewIntrospectionRequest")
	...
	if clientToken := AccessTokenFromRequest(r); clientToken != "" {
		...
		if tu, _, err := p.IntrospectToken(ctx, clientToken, AccessToken, session.Clone()); err != nil {
	...
	tu, ar, err := p.IntrospectToken(ctx, token, TokenUse(tokenTypeHint), session, RemoveEmpty(strings.Split(scope, " "))...)
	if err != nil {
