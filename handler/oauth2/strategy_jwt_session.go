...
func (j *JWTSession) SetExpiresAt(key oauth21.TokenType, exp time.Time) {
	if j.ExpiresAt == nil {
		j.ExpiresAt = make(map[oauth21.TokenType]time.Time)
	}
	j.ExpiresAt[key] = exp
}

func (j *JWTSession) GetExpiresAt(key oauth21.TokenType) time.Time {
	if j.ExpiresAt == nil {
		j.ExpiresAt = make(map[oauth21.TokenType]time.Time)
	}
...
func (j *JWTSession) Clone() oauth21.Session {
	if j == nil {
		return nil
	}

	return deepcopy.Copy(j).(oauth21.Session)
}
