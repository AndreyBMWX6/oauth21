...
func (c *AuthorizeExplicitGrantHandler) HandleTokenEndpointRequest(ctx context.Context, request oauth21.AccessRequester) error {
	if !c.CanHandleTokenEndpointRequest(ctx, request) {
		return errorsx.WithStack(errorsx.WithStack(oauth21.ErrUnknownRequest))
	}

	if !request.GetClient().GetGrantTypes().Has("authorization_code") {
		return errorsx.WithStack(oauth21.ErrUnauthorizedClient.WithHint("The OAuth 2.0 Client is not allowed to use authorization grant \"authorization_code\"."))
	}
	...
	if errors.Is(err, oauth21.ErrInvalidatedAuthorizeCode) {
		if authorizeRequest == nil {
			return oauth21.ErrServerError.
				WithHint("Misconfigured code lead to an error that prohibited the OAuth 2.0 Framework from processing this request.").
				WithDebug("GetAuthorizeCodeSession must return a value for \"oauth21.Requester\" when returning \"ErrInvalidatedAuthorizeCode\".")
		}
		...
		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithHint(hint).WithDebug(debug))
	} else if err != nil && errors.Is(err, oauth21.ErrNotFound) {
	...
	if authorizeRequest.GetClient().GetID() != request.GetClient().GetID() {
		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithHint("The OAuth 2.0 Client ID from this request does not match the one from the authorize request."))
	}
	...
	if forcedRedirectURI != "" && forcedRedirectURI != request.GetRequestForm().Get("redirect_uri") {
		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithHint("The \"redirect_uri\" from this request does not match the one from the authorize request."))
	}
	...
	atLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeAuthorizationCode, oauth21.AccessToken, c.Config.GetAccessTokenLifespan(ctx))
	request.GetSession().SetExpiresAt(oauth21.AccessToken, time.Now().UTC().Add(atLifespan).Round(time.Second))

	rtLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeAuthorizationCode, oauth21.RefreshToken, c.Config.GetRefreshTokenLifespan(ctx))
	if rtLifespan > -1 {
...
func canIssueRefreshToken(ctx context.Context, c *AuthorizeExplicitGrantHandler, request oauth21.Requester) bool {
...
func (c *AuthorizeExplicitGrantHandler) PopulateTokenEndpointResponse(ctx context.Context, requester oauth21.AccessRequester, responder oauth21.AccessResponder) (err error) {
	...
	atLifespan := oauth21.GetEffectiveLifespan(requester.GetClient(), oauth21.GrantTypeAuthorizationCode, oauth21.AccessToken, c.Config.GetAccessTokenLifespan(ctx))
...
func (c *AuthorizeExplicitGrantHandler) CanSkipClientAuth(ctx context.Context, requester oauth21.AccessRequester) bool {
	return false
}

func (c *AuthorizeExplicitGrantHandler) CanHandleTokenEndpointRequest(ctx context.Context, requester oauth21.AccessRequester) bool {
	// grant_type REQUIRED.
