...
import (
	...
	"github.com/AndreyBMWX6/oauth21/storage"
)

var _ oauth21.TokenEndpointHandler = (*RefreshTokenGrantHandler)(nil)
...
func (c *RefreshTokenGrantHandler) HandleTokenEndpointRequest(ctx context.Context, request oauth21.AccessRequester) error {
	if !c.CanHandleTokenEndpointRequest(ctx, request) {
		return errorsx.WithStack(oauth21.ErrUnknownRequest)
	}

	if !request.GetClient().GetGrantTypes().Has("refresh_token") {
		return errorsx.WithStack(oauth21.ErrUnauthorizedClient.WithHint("The OAuth 2.0 Client is not allowed to use authorization grant 'refresh_token'."))
	}
	...
	if errors.Is(err, oauth21.ErrInactiveToken) {
		// Detected refresh token reuse
		if rErr := c.handleRefreshTokenReuse(ctx, signature, originalRequest); rErr != nil {
			return errorsx.WithStack(rErr)
		}

		return errorsx.WithStack(oauth21.ErrInactiveToken.WithWrap(err).WithDebug(err.Error()))
	} else if errors.Is(err, oauth21.ErrNotFound) {
		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithWrap(err).WithDebugf("The refresh token has not been found: %s", err.Error()))
	} else if err != nil {
		...
		if errors.Is(err, oauth21.ErrTokenExpired) {
			return errorsx.WithStack(oauth21.ErrInvalidGrant.WithWrap(err).WithDebug(err.Error()))
		}
		return errorsx.WithStack(oauth21.ErrInvalidRequest.WithWrap(err).WithDebug(err.Error()))
	}
	...
	if !(len(c.Config.GetRefreshTokenScopes(ctx)) == 0 || originalRequest.GetGrantedScopes().HasOneOf(c.Config.GetRefreshTokenScopes(ctx)...)) {
		...
		return errorsx.WithStack(oauth21.ErrScopeNotGranted.WithHint(hint))

	}

	// The authorization server MUST ... and ensure that the refresh token was issued to the authenticated client
	if originalRequest.GetClient().GetID() != request.GetClient().GetID() {
		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithHint("The OAuth 2.0 Client ID from this request does not match the ID during the initial token issuance."))
	}
	...
	atLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeRefreshToken, oauth21.AccessToken, c.Config.GetAccessTokenLifespan(ctx))
	request.GetSession().SetExpiresAt(oauth21.AccessToken, time.Now().UTC().Add(atLifespan).Round(time.Second))

	rtLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeRefreshToken, oauth21.RefreshToken, c.Config.GetRefreshTokenLifespan(ctx))
	if rtLifespan > -1 {
		request.GetSession().SetExpiresAt(oauth21.RefreshToken, time.Now().UTC().Add(rtLifespan).Round(time.Second))
	}
...
func (c *RefreshTokenGrantHandler) PopulateTokenEndpointResponse(ctx context.Context, requester oauth21.AccessRequester, responder oauth21.AccessResponder) (err error) {
	...
	atLifespan := oauth21.GetEffectiveLifespan(requester.GetClient(), oauth21.GrantTypeRefreshToken, oauth21.AccessToken, c.Config.GetAccessTokenLifespan(ctx))
	responder.SetExpiresIn(getExpiresIn(requester, oauth21.AccessToken, atLifespan, time.Now().UTC()))
...
func (c *RefreshTokenGrantHandler) handleRefreshTokenReuse(ctx context.Context, signature string, req oauth21.Requester) (err error) {
	ctx, err = storage.MaybeBeginTx(ctx, c.TokenRevocationStorage)
	if err != nil {
		return errorsx.WithStack(oauth21.ErrServerError.WithWrap(err).WithDebug(err.Error()))
	}
	...
	if err = c.TokenRevocationStorage.DeleteRefreshTokenSession(ctx, signature); err != nil {
		...
		ctx, req.GetID(),
	); err != nil && !errors.Is(err, oauth21.ErrNotFound) {
		return err
...
func (c *RefreshTokenGrantHandler) handleRefreshTokenEndpointStorageError(ctx context.Context, storageErr error) (err error) {
	...
	defer func() {
		if rollBackTxnErr := storage.MaybeRollbackTx(ctx, c.TokenRevocationStorage); rollBackTxnErr != nil {
			err = errorsx.WithStack(oauth21.ErrServerError.WithWrap(err).WithDebugf("error: %s; rollback error: %s", err, rollBackTxnErr))
		}
	}()

	if errors.Is(storageErr, oauth21.ErrSerializationFailure) {
		return errorsx.WithStack(oauth21.ErrInvalidRequest.
	...
	if errors.Is(storageErr, oauth21.ErrNotFound) || errors.Is(storageErr, oauth21.ErrInactiveToken) {
		return errorsx.WithStack(oauth21.ErrInvalidRequest.
			WithDebugf(storageErr.Error()).
			WithHint("Failed to refresh token because of multiple concurrent requests using the same token which is not allowed."))
	}

	return errorsx.WithStack(oauth21.ErrServerError.WithWrap(storageErr).WithDebug(storageErr.Error()))
}

func (c *RefreshTokenGrantHandler) CanSkipClientAuth(ctx context.Context, requester oauth21.AccessRequester) bool {
	return false
}

func (c *RefreshTokenGrantHandler) CanHandleTokenEndpointRequest(ctx context.Context, requester oauth21.AccessRequester) bool {
	// grant_type REQUIRED.
