...
type Handler struct {
	...
	Config interface {
		...
		oauth21.TokenURLProvider
		oauth21.GrantTypeJWTBearerCanSkipClientAuthProvider
		oauth21.GrantTypeJWTBearerIDOptionalProvider
		oauth21.GrantTypeJWTBearerIssuedDateOptionalProvider
		oauth21.GetJWTMaxDurationProvider
...
func (c *Handler) HandleTokenEndpointRequest(ctx context.Context, request oauth21.AccessRequester) error {
	...
	if assertion == "" {
		return errorsx.WithStack(oauth21.ErrInvalidRequest.WithHintf("The assertion request parameter must be set when using grant_type of '%s'.", grantTypeJWTBearer))
	}
	...
	for _, scope := range request.GetRequestedScopes() {
		if !c.Config.GetScopeStrategy(ctx)(scopes, scope) {
			return errorsx.WithStack(oauth21.ErrInvalidScope.WithHintf("The public key registered for issuer \"%s\" and subject \"%s\" is not allowed to request scope \"%s\".", claims.Issuer, claims.Subject, scope))
		}
	}
	...
	atLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeJWTBearer, oauth21.AccessToken, c.HandleHelper.Config.GetAccessTokenLifespan(ctx))
	session.SetExpiresAt(oauth21.AccessToken, time.Now().UTC().Add(atLifespan).Round(time.Second))
	session.SetSubject(claims.Subject)

	return nil
}

func (c *Handler) PopulateTokenEndpointResponse(ctx context.Context, request oauth21.AccessRequester, response oauth21.AccessResponder) error {
	if err := c.CheckRequest(ctx, request); err != nil {
		return err
	}

	atLifespan := oauth21.GetEffectiveLifespan(request.GetClient(), oauth21.GrantTypeJWTBearer, oauth21.AccessToken, c.Config.GetAccessTokenLifespan(ctx))
	return c.IssueAccessToken(ctx, atLifespan, request, response)
}

func (c *Handler) CanSkipClientAuth(ctx context.Context, requester oauth21.AccessRequester) bool {
	return c.Config.GetGrantTypeJWTBearerCanSkipClientAuth(ctx)
}

func (c *Handler) CanHandleTokenEndpointRequest(ctx context.Context, requester oauth21.AccessRequester) bool {
	// grant_type REQUIRED.
...
func (c *Handler) CheckRequest(ctx context.Context, request oauth21.AccessRequester) error {
	...
	if !c.CanSkipClientAuth(ctx, request) && !request.GetClient().GetGrantTypes().Has(grantTypeJWTBearer) {
		return errorsx.WithStack(oauth21.ErrUnauthorizedClient.WithHintf("The OAuth 2.0 Client is not allowed to use authorization grant \"%s\".", grantTypeJWTBearer))
	}
...
func (c *Handler) findPublicKeyForToken(ctx context.Context, token *jwt.JSONWebToken) (*jose.JSONWebKey, error) {
	unverifiedClaims := jwt.Claims{}
	if err := token.UnsafeClaimsWithoutVerification(&unverifiedClaims); err != nil {
		return nil, errorsx.WithStack(oauth21.ErrInvalidRequest.WithWrap(err).WithDebug(err.Error()))
	}
	...
	keyNotFoundErr := oauth21.ErrInvalidGrant.WithHintf(
...
func (c *Handler) validateTokenClaims(ctx context.Context, claims jwt.Claims, key *jose.JSONWebKey) error {
	...
	if claims.ID != "" {
		...
		if used {
			return errorsx.WithStack(oauth21.ErrJTIKnown)
		}
	}
...
func (c *Handler) getSessionFromRequest(requester oauth21.AccessRequester) (extendedSession, error) {
	session := requester.GetSession()
	if jwtSession, ok := session.(extendedSession); !ok {
		return nil, errorsx.WithStack(
			oauth21.ErrServerError.WithHintf("Session must be of type *rfc7523.Session but got type: %T", session),
		)
	} else {
