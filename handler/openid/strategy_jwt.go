...
type DefaultSession struct {
	...
	ExpiresAt map[oauth21.TokenType]time.Time `json:"expires_at"`
...
func (s *DefaultSession) Clone() oauth21.Session {
	if s == nil {
		return nil
	}

	return deepcopy.Copy(s).(oauth21.Session)
}

func (s *DefaultSession) SetExpiresAt(key oauth21.TokenType, exp time.Time) {
	if s.ExpiresAt == nil {
		s.ExpiresAt = make(map[oauth21.TokenType]time.Time)
	}
	s.ExpiresAt[key] = exp
}

func (s *DefaultSession) GetExpiresAt(key oauth21.TokenType) time.Time {
	if s.ExpiresAt == nil {
		s.ExpiresAt = make(map[oauth21.TokenType]time.Time)
	}
...
type DefaultStrategy struct {
	...
	Config interface {
		oauth21.IDTokenIssuerProvider
		oauth21.IDTokenLifespanProvider
		oauth21.MinParameterEntropyProvider
	}
}
...
func (h DefaultStrategy) GenerateIDToken(ctx context.Context, lifespan time.Duration, requester oauth21.Requester) (token string, err error) {
	if lifespan == 0 {
	...
	if requester.GetRequestForm().Get("grant_type") != "refresh_token" {
		...
		if maxAge > 0 {
			if claims.AuthTime.IsZero() {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Failed to generate id token because authentication time claim is required when max_age is set."))
			} else if claims.RequestedAt.IsZero() {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Failed to generate id token because requested at claim is required when max_age is set."))
			} else if claims.AuthTime.Add(time.Second * time.Duration(maxAge)).Before(claims.RequestedAt) {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Failed to generate id token because authentication time does not satisfy max_age time."))
			}
		}
		...
		if prompt != "" {
			if claims.AuthTime.IsZero() {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Unable to determine validity of prompt parameter because auth_time is missing in id token claims."))
			}
		}
		...
		case "none":
			if !claims.AuthTime.Equal(claims.RequestedAt) && claims.AuthTime.After(claims.RequestedAt) {
				return "", errorsx.WithStack(oauth21.ErrServerError.
		...
		if tokenHintString := requester.GetRequestForm().Get("id_token_hint"); tokenHintString != "" {
			...
			if errors.As(err, &ve) && ve.Has(jwt.ValidationErrorExpired) {
				...
				return "", errorsx.WithStack(oauth21.ErrServerError.WithWrap(err).WithDebugf("Unable to decode id token from 'id_token_hint' parameter because %s.", err.Error()))
			}

			if hintSub, _ := tokenHint.Claims["sub"].(string); hintSub == "" {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Provided id token from 'id_token_hint' does not have a subject."))
			} else if hintSub != claims.Subject {
				return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Subject from authorization mismatches id token subject from 'id_token_hint'."))
			}
		}
	}
	...
	if claims.ExpiresAt.Before(time.Now().UTC()) {
		return "", errorsx.WithStack(oauth21.ErrServerError.WithDebug("Failed to generate id token because expiry claim can not be in the past."))
	}
	...
	if nonce := requester.GetRequestForm().Get("nonce"); len(nonce) == 0 {
		...
		return "", errorsx.WithStack(oauth21.ErrInsufficientEntropy.WithHintf("Parameter 'nonce' is set but does not satisfy the minimum entropy of %d characters.", h.Config.GetMinParameterEntropy(ctx)))
	} else if len(nonce) > 0 {
		claims.Nonce = nonce
	}
