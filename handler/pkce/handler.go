...
type Handler struct {
	...
	Config                interface {
		oauth21.EnforcePKCEProvider
		oauth21.EnforcePKCEForPublicClientsProvider
		oauth21.EnablePKCEPlainChallengeMethodProvider
	}
}
...
func (c *Handler) HandleAuthorizeEndpointRequest(ctx context.Context, ar oauth21.AuthorizeRequester, resp oauth21.AuthorizeResponder) error {
	...
	code := resp.GetCode()
	if len(code) == 0 {
		return errorsx.WithStack(oauth21.ErrServerError.WithDebug("The PKCE handler must be loaded after the authorize code handler."))
	}
...
func (c *Handler) validate(ctx context.Context, challenge, method string, client oauth21.Client) error {
	if len(challenge) == 0 {
...
func (c *Handler) validateNoPKCE(ctx context.Context, client oauth21.Client) error {
...
func (c *Handler) HandleTokenEndpointRequest(ctx context.Context, request oauth21.AccessRequester) error {
	...
	if errors.Is(err, oauth21.ErrNotFound) {
		if nv == 0 {
			return c.validateNoPKCE(ctx, request.GetClient())
		}

		return errorsx.WithStack(oauth21.ErrInvalidGrant.WithHint("Unable to find initial PKCE data tied to this request").WithWrap(err).WithDebug(err.Error()))
	} else if err != nil {
		return errorsx.WithStack(oauth21.ErrServerError.WithWrap(err).WithDebug(err.Error()))
	}
...
func (c *Handler) PopulateTokenEndpointResponse(ctx context.Context, requester oauth21.AccessRequester, responder oauth21.AccessResponder) error {
	return nil
}
