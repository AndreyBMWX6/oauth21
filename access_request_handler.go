...
//     in Section 3.2.1.
func (p *OAuth2Provider) NewAccessRequest(ctx context.Context, r *http.Request, session Session) (_ AccessRequester, err error) {
	ctx, span := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/AndreyBMWX6/oauth21").Start(ctx, "OAuth2Provider.NewAccessRequest")
	defer otelx.End(span, &err)

	accessRequest := NewAccessRequest(session)
	accessRequest.Request.Lang = i18n.GetLangFromRequest(p.Config.GetMessageCatalog(ctx), r)
	...
	client, clientErr := p.AuthenticateClient(ctx, r, r.PostForm)
	if clientErr == nil {
		accessRequest.Client = client
	}

	var found = false
	for _, loader := range p.Config.GetTokenEndpointHandlers(ctx) {
