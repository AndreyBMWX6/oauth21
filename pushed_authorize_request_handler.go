...
func (p *OAuth2Provider) NewPushedAuthorizeRequest(ctx context.Context, r *http.Request) (_ AuthorizeRequester, err error) {
	ctx, span := trace.SpanFromContext(ctx).TracerProvider().Tracer("github.com/AndreyBMWX6/oauth21").Start(ctx, "OAuth2Provider.NewPushedAuthorizeRequest")
	...
	client, err := p.AuthenticateClient(ctx, r, r.Form)
	if err != nil {
	...
	fr, err := p.newAuthorizeRequest(ctx, r, true)
	if err != nil {
		return fr, err
	}
