...
func (p *OAuth2Provider) authorizeRequestParametersFromOpenIDConnectRequest(ctx context.Context, request *AuthorizeRequest, isPARRequest bool) error {
	...
	token, err := jwt.ParseWithClaims(assertion, jwt.MapClaims{}, func(t *jwt.Token) (interface{}, error) {
		...
		case jose.RS256, jose.RS384, jose.RS512:
			key, err := p.findClientPublicJWK(ctx, oidcClient, t, true)
			if err != nil {
		...
		case jose.ES256, jose.ES384, jose.ES512:
			key, err := p.findClientPublicJWK(ctx, oidcClient, t, false)
			if err != nil {
				return nil, wrapSigningKeyFailure(
...
func (p *OAuth2Provider) validateAuthorizeRedirectURI(_ *http.Request, request *AuthorizeRequest) error {
...
func (p *OAuth2Provider) parseAuthorizeScope(_ *http.Request, request *AuthorizeRequest) error {
	request.SetRequestedScopes(RemoveEmpty(strings.Split(request.Form.Get("scope"), " ")))

	return nil
}

func (p *OAuth2Provider) validateAuthorizeScope(ctx context.Context, _ *http.Request, request *AuthorizeRequest) error {
	for _, permission := range request.GetRequestedScopes() {
		if !p.Config.GetScopeStrategy(ctx)(request.Client.GetScopes(), permission) {
...
func (p *OAuth2Provider) validateResponseTypes(r *http.Request, request *AuthorizeRequest) error {
...
func (p *OAuth2Provider) ParseResponseMode(ctx context.Context, r *http.Request, request *AuthorizeRequest) error {
	...
	default:
		...
		if p.ResponseModeHandler(ctx).ResponseModes().Has(rm) {
...
func (p *OAuth2Provider) validateResponseMode(r *http.Request, request *AuthorizeRequest) error {
...
func (p *OAuth2Provider) authorizeRequestFromPAR(ctx context.Context, r *http.Request, request *AuthorizeRequest) (bool, error) {
	configProvider, ok := p.Config.(PushedAuthorizeRequestConfigProvider)
	if !ok {
	...
	storage, ok := p.Storage.(PARStorage)
	if !ok {
...
func (p *OAuth2Provider) newAuthorizeRequest(ctx context.Context, r *http.Request, isPARRequest bool) (AuthorizeRequester, error) {
	request := NewAuthorizeRequest()
	request.Request.Lang = i18n.GetLangFromRequest(p.Config.GetMessageCatalog(ctx), r)
	...
	if !isPARRequest {
		if isPAR, err := p.authorizeRequestFromPAR(ctx, r, request); err != nil {
			return request, err
		} else if isPAR {
			// No need to continue
			return request, nil
		} else if configProvider, ok := p.Config.(PushedAuthorizeRequestConfigProvider); ok && configProvider.EnforcePushedAuthorize(ctx) {
	...
	client, err := p.Storage.GetClient(ctx, request.GetRequestForm().Get("client_id"))
	if err != nil {
	...
	if err := p.authorizeRequestParametersFromOpenIDConnectRequest(ctx, request, isPARRequest); err != nil {
		return request, err
	}

	// The request context is now fully available and we can start processing the individual
	// fields.
	if err := p.ParseResponseMode(ctx, r, request); err != nil {
		return request, err
	}

	if err = p.parseAuthorizeScope(r, request); err != nil {
		return request, err
	}

	if err = p.validateAuthorizeRedirectURI(r, request); err != nil {
		return request, err
	}

	if err = p.validateAuthorizeScope(ctx, r, request); err != nil {
		return request, err
	}

	if err = p.validateAuthorizeAudience(ctx, r, request); err != nil {
		return request, err
	}
	...
	if err = p.validateResponseTypes(r, request); err != nil {
		return request, err
	}

	if err = p.validateResponseMode(r, request); err != nil {
		return request, err
	}
	...
	if len(request.State) < p.GetMinParameterEntropy(ctx) {
		// We're assuming that using less then, by default, 8 characters for the state can not be considered "unguessable"
		return request, errorsx.WithStack(ErrInvalidState.WithHintf("Request parameter 'state' must be at least be %d characters long to ensure sufficient entropy.", p.GetMinParameterEntropy(ctx)))
	}
