...
type AudienceMatchingStrategy func(audiences []string, audience []string) error

func DefaultAudienceMatchingStrategy(audiences []string, audience []string) error {
	if len(audience) == 0 {
		return nil
	}

	for _, n := range audience {
		nu, err := url.Parse(n)
		if err != nil {
...
func (p *OAuth2Provider) validateAuthorizeAudience(ctx context.Context, r *http.Request, request *AuthorizeRequest) error {
	audience := GetAudiences(request.Form)

	if err := p.Config.GetAudienceStrategy(ctx)(request.Client.GetAudience(), audience); err != nil {
		return err
	}
